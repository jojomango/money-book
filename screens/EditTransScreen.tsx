...
const EditTransScreen = ({ navigation, route }) => {
  ...
  const getBookOption = (bookId: string) =>
    bookOptions.find((b) => b.value === bookId);

  let editRecord: Record;
  if (transId && transBookId) {
    editRecord = useSelector((state) =>
      state.transactions[transBookId].records.find(
        (record) => record.transId === transId,
      ),
    );
  }
  ...
  const [formState, dispatchFormState] = useReducer(
    formReducer,
    { editRecord, defaultBookId, transBookId },
    initFormstate,
  );
  ...
  const submitHandler = useCallback(() => {
    if (transId) {
      dispatch(
        updateTransaction(
          { ...editRecord, ...formState.inputValues },
          transId,
          transBookId,
        ),
      );
        ...
        <ScrollView>
          <View style={styles.form}>
            ...
            <Picker
              ...
              items={bookOptions}
              onItemChange={(option) => {
                inputChangedHandler("bookId", option.value, true);
              }}
              title="Select Book"
